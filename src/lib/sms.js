...
const formParse = (obj, auth, uri) => {
  return new Promise((resolve, reject) => {
    if (
      ...
      obj.headers["content-type"].indexOf("multipart/form-data") !== -1
    ) {
...
const postRequest = (data) => {
  ...
  for (let key in data) {
    if (key == "image" || key == "fimage") {
      // 파일만 별도로 담아주기
      form.append(key, fs.createReadStream(data[key][0].path), {
        filename: data[key][0].originalFilename,
        contentType: data[key][0].headers["content-type"],
      });
    } else {
